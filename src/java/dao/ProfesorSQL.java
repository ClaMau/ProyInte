...
public class ProfesorSQL{
    ...
    public void agregar(profesor p) {
        ...
        try{
            ...
            st.setInt(1, p.getIdHorario());
            st.setString(2, p.getCargo());
            st.setString(3, p.getGenero());
            st.setString(4, p.getNroDocu());
            st.setString(5, p.getNombres());
            st.setString(6, p.getApePat());
            st.setString(7, p.getApeMat());
            st.setString(8, p.getTelf());
            st.setString(9, p.getCorreo());
            st.setString(10, p.getCurso());
            st.executeUpdate();
            }
    ...
    public void actualizar(asistencias a){
       Connection cn=MySQLConexion.getConexion();
      try{
            String sql="update asistencias set id_usuario=?,fecha_entrada=?,hora_entrada=?,fecha_salida=?,hora_salida? where id=?";
            PreparedStatement st=cn.prepareStatement(sql);
      ...
      public profesor BuscarProf(String documento) {
        profesor p = null;
         Connection cn=MySQLConexion.getConexion();
        try {
            String sql = "select id,id_horario,nom_cargo,genero,nro_documento,nombres,"
      ...
      public profesor Busca(String documento){
          ...
          try{
              String sql="select id from colegio where nro_documento=?";
              PreparedStatement st=cn.prepareStatement(sql);
              st.setString(1, documento);
              ResultSet rs=st.executeQuery();
              if(rs.next()){
